...
# Task ordering and percent milestones (FOCUS MODE: Buyer Migration only)
TASK_ORDER = [
    "location",
    "buyer_migration",
]
...
async def analyze_zip_code(request: ZipAnalysisRequest, background_tasks: BackgroundTasks):
    """FOCUS MODE: Generate only Buyer Migration and return."""
    try:
...
async def start_zip_analysis(request: ZipAnalysisRequest):
    """Start analysis job (Buyer Migration only) and return status."""
    zip_code = request.zip_code
    existing = await db.market_intelligence.find_one({
        "zip_code": zip_code,
        "created_at": {"$gte": datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)}
...
# The PDF and content asset endpoints remain unchanged
